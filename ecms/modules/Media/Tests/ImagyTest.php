...
class ImagyTest extends MediaTestCase
{
    ...
    {
        parent::setUp();
        $this->app['config']->set('encore.media.config.files-path', '/assets/media/');
    ...
    public function it_should_create_a_file()
    {
        ...
        $this->finder->delete(public_path(config('encore.media.config.files-path') . 'my-file_smallThumb.jpg'));

        $this->imagy->get($file->path, 'smallThumb', true);

        $this->assertTrue($this->finder->isFile(public_path(config('encore.media.config.files-path') . 'my-file_smallThumb.jpg')));
    }
    ...
    public function it_should_return_thumbnail_path()
    {
        ...
        $expected = config('app.url') . config('encore.media.config.files-path') . 'my-file_smallThumb.jpg';
    ...
    public function it_should_return_thumbnail_path_to_sub_folders()
    {
        ...
        $expected = config('app.url') . config('encore.media.config.files-path') . 'my-folder/my-file_smallThumb.jpg';
    ...
    public function it_can_delete_thumbnail_files()
    {
        ...
        $this->assertCount(3, $this->app['files']->allFiles(public_path(config('encore.media.config.files-path'))));
        $this->imagy->deleteAllFor($file);
        $this->assertCount(0, $this->app['files']->allFiles(public_path(config('encore.media.config.files-path'))));
    }
    ...
    public function it_can_delete_thumbnail_files_in_subdirectory()
    {
        ...
        $path = public_path(config('encore.media.config.files-path') . 'my-folder/');
    ...
    public function it_can_delete_a_non_image_file()
    {
        ...
        $this->assertCount(1, $this->app['files']->allFiles(public_path(config('encore.media.config.files-path'))));
        $this->imagy->deleteAllFor($file);
        $this->assertCount(0, $this->app['files']->allFiles(public_path(config('encore.media.config.files-path'))));
    }
