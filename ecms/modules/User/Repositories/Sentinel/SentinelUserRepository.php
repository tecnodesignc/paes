...
class SentinelUserRepository implements UserRepository
{
    ...
    public function create(array $data, $activated = false)
    {
        ...
        $user->companies()->sync(Arr::get($data, 'companies', []));
    ...
    public function getItemsBy($params)
    {
        ...
        if (isset($params->filter)) {
            ...
            //filter by Roles
            if (isset($filter->companies) && count($filter->companies)) {
                $query->whereHas('companies', function (Builder $q) use ($filter) {
                    $q->whereIn('company_id', $filter->companies);
                });
            }
