...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $parameters=json_decode(json_encode(['filter'=>['order'=>['field'=>$request->input('order'),'way'=>$request->input('way')],'roles'=>$request->input('roles'),'search'=>$request->input('search'),'companies'=>$request->input('companies')],'include'=>$includes,'page'=>$request->input('page'),'take'=>$request->input('limit')]));

        return UserTransformer::collection($this->user->getItemsBy($parameters));
    }
